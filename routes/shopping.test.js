...
describe("GET /shop", async function () {
...
describe("GET /shop/:name", async function () {
  test("Gets a single item", async function () {
    const response = await request(app).get(`/shop/${item.name}`);
    expect(response.statusCode).toBe(200);
    expect(response.body.item).toEqual(item);
  });

  test("Responds with 404 if can't find item", async function () {
    const response = await request(app).get(`/shop/0`);
...
describe("POST /shop", async function () {
...
describe("PATCH /shop/:name", async function () {
  test("Updates a single item", async function () {
    const response = await request(app)
      .patch(`/shop/${item.name}`)
      .send({
        name: "Troll"
      });
    expect(response.statusCode).toBe(200);
  ...
  test("Responds with 404 if can't find item", async function () {
    const response = await request(app).patch(`/shop/0`);
    expect(response.statusCode).toBe(404);
  });
});
// end
...
describe("DELETE /shop/:name", async function () {
